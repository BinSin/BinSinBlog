...
SVG 는 각각의 그래픽 요소들이 DOM 객체를 만드는 Vector 기반 그래픽스이고 HTML5 는 자바스크립트 API 기반 이미지를 만드는 Raster 기반 그래픽스이다.
  ...
  - 공통점
    ...
    2. 2D로 그림을 그릴 수 있음
  - 차이점
    1. Canvas 는 복잡한 이미지를 허용한다.
    2. Canvas 는 지속적인 성능을 제공한다.
    3. Canvas 는 무언가 변경되면 모두 다시 그린다.
    4. SVG 는 이벤트 첨부를 허용한다.
    5. SVG 는 animation API 를 허용한다.
    6. DOM 요소가 증가하면 SVG 의 속도가 느려진다.
